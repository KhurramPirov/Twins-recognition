...
restored_image - повернутое на обнаруженный угол изображение. Дополняется нулями. Конретный угол, на который БЫЛО повернуто лицо можно получить из `pcn.get_angle()` (в градусах). `visualize=True` для плоттинга картинок.
Если не задетектировало лицо - возвращает исходную картинку и 0 в качесве угла. 
