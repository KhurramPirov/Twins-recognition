...
#             kp1 = kp1.cuda()
#             kp2 = kp2.cuda()
        emb_1 = model(images_1)#, kp1)
        emb_2 = model(images_2)#, kp2)
...
#         print('Face recognition model config:')
#         print(model)
#         print('Number of parameters: {}'.format(get_model_parameters_number(model)))
    ...
    for subset in tqdm(subsets, '{} evaluation'.format(snap_name), disable=not verbose):
        ...
        for score_with_gt in test_list:
            running_val.append((score_with_gt['score'] < optimal_thresh) == score_with_gt['is_same'])
        ...
        val_scores.append(np.mean(running_val))
#         print(len(running_val))
#         val_scores.append(0.5*(same_scores[-1] + diff_scores[-1]))
    ...
    t_idx = np.argmax(val_scores)
    overall_acc = np.mean(val_scores)
    auc = get_auc(mean_fpr, mean_tpr)

    if show_failed:
        print('Number of misclassified pairs: {}'.format(len(failed_pairs)))
        for pair in failed_pairs:
            dataset.show_item(pair)

    if verbose:
        print('Accuracy/Val_same_accuracy mean: {0:.4f}'.format(same_acc))
        print('Accuracy/Val_diff_accuracy mean: {0:.4f}'.format(diff_acc))
        print('Accuracy/Val_accuracy mean: {0:.4f}'.format(overall_acc))
        print('Accuracy/Val_accuracy std dev: {0:.4f}'.format(np.std(val_scores)))
        print('AUC: {0:.4f}'.format(auc))
        print('Estimated threshold: {0:.4f}'.format(threshs[t_idx]))
#         print('Estimated threshold: {0:.4f}'.format(np.mean(threshs)))
...
def load_test_dataset(arguments):
    ...
    print('Using landmarks for the LFW images.')
